...
# Lists    
flourTypeList = ['none', 'BL55 buzafinomliszt', 'BL80 kenyerliszt', 'durumliszt', 'rozsliszt', 
...
# Recipe calculator    
def calculateRecipe():
    g_flour_inputLeaven = float(e_leavenShare.get())/100/(1+float(e_leavenHydration.get())/100)
    g_flour_inputPoolish = float(e_poolishShare.get())/100/(1+float(e_poolishHydration.get())/100)
    g_flour_inputTangzhong = float(e_tangzhongShare.get())/100/6    
    g_flour = float(e_loafWeight.get())/(1+g_flour_inputLeaven+g_flour_inputPoolish+g_flour_inputTangzhong)
    g_leaven = g_flour*float(e_leavenShare.get())/100
    g_poolish = g_flour*float(e_poolishShare.get())/100
    g_tangzhong = g_flour*float(e_tangzhongShare.get())/100
    g_hydration = float(e_loafWeight.get())*float(e_hydrationPerc.get())/100-(g_leaven*(float(e_leavenHydration.get())/(100 +float(e_leavenHydration.get())))+
                                                                              g_poolish*(float(e_poolishHydration.get())/(100 +float(e_poolishHydration.get())))+
                                                                              g_tangzhong*5/6)
    ...
    e_poolishAmount.delete(0, END)
    e_poolishAmount.insert(0, round(g_poolish, 0))
    e_tangzhongAmount.delete(0, END)
    e_tangzhongAmount.insert(0, round(g_tangzhong, 0))
    ...
    g_flourTypeWeight1 = (g_flour+g_tangzhong/6)*float(e_flourTypePerc1.get())/100-g_tangzhong/6
    g_flourTypeWeight2 = (g_flour+g_tangzhong/6)*float(e_flourTypePerc2.get())/100
    g_flourTypeWeight3 = (g_flour+g_tangzhong/6)*float(e_flourTypePerc3.get())/100
...
def prefrmt_getAmounts():
    e_prefrmt_leavenAmount.delete(0, END)
    e_prefrmt_leavenAmount.insert(0, e_leavenAmount.get())
    e_prefrmt_leavenHydration.delete(0, END)
    e_prefrmt_leavenHydration.insert(0, e_leavenHydration.get())
    e_prefrmt_poolishAmount.delete(0, END)
    e_prefrmt_poolishAmount.insert(0, e_poolishAmount.get())
    e_prefrmt_poolishHydration.delete(0, END)
    e_prefrmt_poolishHydration.insert(0, e_poolishHydration.get())    
    e_prefrmt_tangzhongAmount.delete(0, END)
    e_prefrmt_tangzhongAmount.insert(0, e_tangzhongAmount.get())

    
def prefrmt_addAmounts():
    e_leavenAmount.delete(0, END)
    e_leavenAmount.insert(0, e_prefrmt_leavenAmount.get())
    e_leavenHydration.delete(0, END)
    e_leavenHydration.insert(0, e_prefrmt_leavenHydration.get())
    e_poolishAmount.delete(0, END)
    e_poolishAmount.insert(0, e_prefrmt_poolishAmount.get())
    e_poolishHydration.delete(0, END)
    e_poolishHydration.insert(0, e_prefrmt_poolishHydration.get())    
    e_tangzhongAmount.delete(0, END)
    e_tangzhongAmount.insert(0, e_prefrmt_tangzhongAmount.get())
    
    
def prefrmt_calculateWeights():
    if(e_prefrmt_leavenAmount.get()=="0" or e_prefrmt_leavenAmount.get()=="0.0" or e_prefrmt_leavenAmount.get()==""):
        e_flourTypeLeavenWeight1.delete(0, END)
        e_flourTypeLeavenWeight1.insert(0, 0)
        e_flourTypeLeavenWeight2.delete(0, END)
        e_flourTypeLeavenWeight2.insert(0, 0)
        e_prefrmt_leavenHydrationWeight.delete(0, END)
        e_prefrmt_leavenHydrationWeight.insert(0, 0)
    else:
        total_perc = float(e_flourTypeLeaven1.get())+float(e_flourTypeLeaven2.get())+float(e_prefrmt_leavenHydration.get())
        g_flourTypeLeaven1 = float(e_prefrmt_leavenAmount.get())/total_perc*float(e_flourTypeLeaven1.get())
        g_flourTypeLeaven2 = float(e_prefrmt_leavenAmount.get())/total_perc*float(e_flourTypeLeaven2.get())
        g_prefrmt_leavenHydration = float(e_prefrmt_leavenAmount.get())/total_perc*float(e_prefrmt_leavenHydration.get())
        e_flourTypeLeavenWeight1.delete(0, END)
        e_flourTypeLeavenWeight1.insert(0, round(g_flourTypeLeaven1, 0))
        e_flourTypeLeavenWeight2.delete(0, END)
        e_flourTypeLeavenWeight2.insert(0, round(g_flourTypeLeaven2, 0))
        e_prefrmt_leavenHydrationWeight.delete(0, END)
        e_prefrmt_leavenHydrationWeight.insert(0, round(g_prefrmt_leavenHydration, 0))
    if(e_prefrmt_poolishAmount.get()=="0" or e_prefrmt_poolishAmount.get()=="0.0" or e_prefrmt_poolishAmount.get()==""):
        e_flourTypePoolishWeight1.delete(0, END)
        e_flourTypePoolishWeight1.insert(0, 0)
        e_flourTypePoolishWeight2.delete(0, END)
        e_flourTypePoolishWeight2.insert(0, 0)
        e_prefrmt_poolishHydrationWeight.delete(0, END)
        e_prefrmt_poolishHydrationWeight.insert(0, 0)
        e_prefrmt_poolish_leavenAmount.delete(0, END)
        e_prefrmt_poolish_leavenAmount.insert(0, 0)
    else:
        g_poolish_dry = float(e_prefrmt_poolishAmount.get())/(100+float(e_prefrmt_poolishHydration.get()))*100
        g_poolish_wet = float(e_prefrmt_poolishAmount.get())/(100+float(e_prefrmt_poolishHydration.get()))*float(e_prefrmt_poolishHydration.get())
        poolish_dry_totalPerc = (float(e_flourTypePoolish1.get())+float(e_flourTypePoolish2.get())+
                                 float(e_prefrmt_poolish_leavenShare.get())/(100+float(e_prefrmt_poolish_leavenHydration.get()))*100)
        g_flourTypePoolish1 = g_poolish_dry/poolish_dry_totalPerc*float(e_flourTypePoolish1.get())
        g_flourTypePoolish2 = g_poolish_dry/poolish_dry_totalPerc*float(e_flourTypePoolish2.get())
        poolish_wet_totalPerc = float(e_prefrmt_poolishHydration.get())+float(e_prefrmt_poolish_leavenShare.get())/(100+float(e_prefrmt_poolish_leavenHydration.get()))*float(e_prefrmt_poolish_leavenHydration.get())
        g_prefrmt_poolishHydration = g_poolish_wet/poolish_wet_totalPerc*float(e_prefrmt_poolishHydration.get())
        g_prefrmt_poolish_leavenAmount = (g_flourTypePoolish1+g_flourTypePoolish2)*float(e_prefrmt_poolish_leavenShare.get())/100
        e_flourTypePoolishWeight1.delete(0, END)
        e_flourTypePoolishWeight1.insert(0, round(g_flourTypePoolish1, 0))
        e_flourTypePoolishWeight2.delete(0, END)
        e_flourTypePoolishWeight2.insert(0, round(g_flourTypePoolish2, 0))
        e_prefrmt_poolishHydrationWeight.delete(0, END)
        e_prefrmt_poolishHydrationWeight.insert(0, round(g_prefrmt_poolishHydration, 0))
        e_prefrmt_poolish_leavenAmount.delete(0, END)
        e_prefrmt_poolish_leavenAmount.insert(0, round(g_prefrmt_poolish_leavenAmount, 0))
    if(e_prefrmt_tangzhongAmount.get()=="0" or e_prefrmt_tangzhongAmount.get()=="0.0" or e_prefrmt_tangzhongAmount.get()==""):
        e_flourTypeTangzhongWeight.delete(0, END)
        e_flourTypeTangzhongWeight.insert(0, 0)
        e_prefrmt_tangzhongHydrationWeight.delete(0, END)
        e_prefrmt_tangzhongHydrationWeight.insert(0, 0)
    else:        
        g_flourTypeTangzhong = float(e_prefrmt_tangzhongAmount.get())/6
        g_prefrmt_tangzhongHydration = float(e_prefrmt_tangzhongAmount.get())/6*5
        e_flourTypeTangzhongWeight.delete(0, END)
        e_flourTypeTangzhongWeight.insert(0, round(g_flourTypeTangzhong, 0))
        e_prefrmt_tangzhongHydrationWeight.delete(0, END)
        e_prefrmt_tangzhongHydrationWeight.insert(0, round(g_prefrmt_tangzhongHydration, 0))
    
    
def prefrmt_calculatePerc():
    return;    
...
def breadSelect(evt):
    ...
    e_poolishShare.delete(0, END)
    e_poolishShare.insert(0, selectedData['e_poolishShare'][0])
    e_poolishHydration.delete(0, END)
    e_poolishHydration.insert(0, selectedData['e_poolishHydration'][0])
    e_tangzhongShare.delete(0, END)
    e_tangzhongShare.insert(0, selectedData['e_tangzhongShare'][0])
    ...
    flourTypePoolish1.set(selectedData['flourTypePoolish1'][0])
    flourTypePoolish2.set(selectedData['flourTypePoolish2'][0])
    e_flourTypePoolish1.delete(0, END)
    e_flourTypePoolish1.insert(0, selectedData['e_flourTypePoolish1'][0])
    e_flourTypePoolish2.delete(0, END)
    e_flourTypePoolish2.insert(0, selectedData['e_flourTypePoolish2'][0])
    e_prefrmt_poolish_leavenShare.delete(0, END)
    e_prefrmt_poolish_leavenShare.insert(0, selectedData['e_prefrmt_poolish_leavenShare'][0])
    e_prefrmt_poolish_leavenHydration.delete(0, END)
    e_prefrmt_poolish_leavenHydration.insert(0, selectedData['e_prefrmt_poolish_leavenHydration'][0])
    flourTypeTangzhong.set(selectedData['flourTypeTangzhong'][0])
...
def saveRecipeYes():
    ...
    dataCollect = {'breadName': recipeName, 
            ...
            'e_poolishShare':float(e_poolishShare.get()),
            'e_poolishHydration':float(e_poolishHydration.get()),
            'e_tangzhongShare':float(e_tangzhongShare.get()),
            ...
            'flourTypePoolish1':flourTypePoolish1.get(),
            'flourTypePoolish2':flourTypePoolish2.get(),
            'e_flourTypePoolish1':float(e_flourTypePoolish1.get()),
            'e_flourTypePoolish2':float(e_flourTypePoolish1.get()),
            'e_prefrmt_poolish_leavenShare':float(e_prefrmt_poolish_leavenShare.get()),
            'e_prefrmt_poolish_leavenHydration':float(e_prefrmt_poolish_leavenHydration.get()),
            'flourTypeTangzhong':flourTypeTangzhong.get(),
